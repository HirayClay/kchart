...
class RsiRenderer(override val properties: RsiRendererProperties, override val bitMartChartView: IBitMartChartView) : BaseRenderer<RsiRendererProperties>() {
    ...
    override val rangeMaxBy: (dataEntity: ChartDataEntity) -> Double = { dataEntity -> maxOf(dataEntity.rsi[0], dataEntity.rsi[1], dataEntity.rsi[2]) }

    override val rangeMinBy: (dataEntity: ChartDataEntity) -> Double = { dataEntity -> minOf(dataEntity.rsi[0], dataEntity.rsi[1], dataEntity.rsi[2]) }
    ...
    override fun drawData(dataRect: RectF, canvas: Canvas, min: Double, max: Double, itemWidth: Float, preStartX: Float?, preData: ChartDataEntity?, curStartX: Float, curData: ChartDataEntity) {
        ...
        drawLine(canvas, min, max, itemWidth, preStartX, preData?.rsi?.get(0), curStartX, curData.rsi[0])
        linePaint.color = getIndexColor()[1]
        drawLine(canvas, min, max, itemWidth, preStartX, preData?.rsi?.get(1), curStartX, curData.rsi[1])
        linePaint.color = getIndexColor()[2]
        drawLine(canvas, min, max, itemWidth, preStartX, preData?.rsi?.get(2), curStartX, curData.rsi[2])
    }
